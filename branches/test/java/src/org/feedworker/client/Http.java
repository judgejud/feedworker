...
class Http {
    ...
    private final String TAG_SUBSF =
            "<td align=\"center\" class=\"functions\" width=\"100%\"><a href";
     ...
     * @param password
     */
    private void itasaLogonPOST(String username, String password) throws IOException{
    ...
    InputStream getTorrent(String oldUrl) throws IOException{
        InputStream is = null;        
        if (oldUrl.substring(0, TORRENT_ZOINK.length()).equalsIgnoreCase(TORRENT_ZOINK)){
            namefile = oldUrl.substring(TORRENT_ZOINK.length());
            get = new HttpGet(convertUrlEncoding(oldUrl));
            response = client.execute(get);
            is = response.getEntity().getContent();
        } else if (oldUrl.substring(0, TORRENT_MININOVA.length()).
                equalsIgnoreCase(TORRENT_MININOVA)){
            String url = oldUrl.replace("tor","get");
            get = new HttpGet(url);
            response = client.execute(get);
            getAttachement(response.getAllHeaders(), TORRENT_MININOVA);
            is = response.getEntity().getContent();
        } else if (oldUrl.substring(0, TORRENT_BTCHAT.length()).
                equalsIgnoreCase(TORRENT_BTCHAT)){
    ...
    String synoConnectGetID(String url, String user, String pwd) throws IOException{
        String id = null;
        String searchID = "   \"id\" : \"";
        List<NameValuePair> lnvp = new ArrayList<NameValuePair>();
        ...
        String line;
        while ((line = br.readLine()) != null) {
            if (line.length()>11 && line.substring(0, 11).equalsIgnoreCase(searchID)){
    ...
    InputStream synoStatus(String url, String id) throws IllegalStateException, IOException{
        List<NameValuePair> lnvp = new ArrayList<NameValuePair>();
        lnvp.add(new BasicNameValuePair("id",id));
