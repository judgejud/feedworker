...
public class Kernel implements PropertyChangeListener {
			...
			if (ist != null) {
				...
				if (data != null) {
					...
					for (int i = matrice.size() - 1; i >= 0; i--) {
						...
						if (confronta
								.before(Common.stringDateTime(date_matrix))) {
							if (continua) {
	...
	private Object[] setArray(TvRage t, Object[] show, Itasa it)
		...
		Object[] array = t.readingEpisodeList_byID(show[0].toString(),
				show[2].toString());
		ArrayList<String> temp = it.searchIdSingleByTvrage(Integer
				.parseInt(show[0].toString()));
		array[0] = show[0];
		if (temp != null) {
			array[1] = temp.get(0);
			array[2] = temp.get(1);
		...
		if (evt.getSource().getClass().getName()
			...
			if (evt.getPropertyName().equals("progress")) {
				...
				if (importTask.isDone() && !importTask.isCancelled()) {
					try {
						ManageListener.fireTableEvent(this, importTask.get(),
								CALENDAR);
					} catch (Exception e) {
						error.launch(e);
					}
	...
	public void refreshCalendar() {
		...
		try {
			array = XPathReader.queryDayID(Common.dateString(Common
					.actualDate()));
