...
public class Kernel implements PropertyChangeListener {
	...
	// public final static String[] daysOfWeek = new String[]{"", "Domenica",
	// "Lunedì",
	...
	private void analyzeVideoSamba(Samba s, String[] fileList) {
		for (int i = 0; i < fileList.length; i++) {
			...
			if (name.toLowerCase().contains(SPLIT_HDTV)) {
				...
				if (dest != null) {
					try {
						String[] _array = name.toLowerCase().split(SPLIT_HDTV)[0]
								.split("\\.");
		...
		public ArrayList<Object[]> doInBackground() {
			...
			try {
				while (iter.hasNext()) {
					...
					if (!name.toLowerCase().equalsIgnoreCase(
							oldName.toLowerCase())) {
						ArrayList<Object[]> temp = t
								.readingDetailedSearch_byShow(name, true);
						if (temp != null) {
	...
	private synchronized void fireNewTextPaneEvent(String msg, String type) {
		...
		while (listeners.hasNext()) {
			TextPaneEventListener myel = (TextPaneEventListener) listeners
					.next();
			myel.objReceived(event);
		}
	}
	...
	private synchronized void fireJFrameEventOperation(String oper) {
		...
		while (listeners.hasNext()) {
			JFrameEventOperationListener myel = (JFrameEventOperationListener) listeners
					.next();
			myel.objReceived(event);
		}
	}
	...
	private synchronized void fireNewJFrameEventIconData(boolean _icontray,
		...
		while (listeners.hasNext()) {
			JFrameEventIconDateListener myel = (JFrameEventIconDateListener) listeners
					.next();
			myel.objReceived(event);
		}
		/*
