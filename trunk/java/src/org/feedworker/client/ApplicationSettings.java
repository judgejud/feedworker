...
public class ApplicationSettings {
		...
		try {
			if (SETTINGS_FILE.exists()) {
				...
				setAutoDownload(Boolean
						.parseBoolean(getDecryptedValue("IS_AUTO_DOWNLOAD")));
	...
	public void writeGeneralSettings() {
		try {
			properties.setProperty(propertyEncrypter
					.encrypt("SUBTITLE_DESTINATION_FOLDER"), valueEncrypter
					.encrypt(subtitleDestinationFolder));
			properties.setProperty(propertyEncrypter
					.encrypt("REFRESH_INTERVAL"), valueEncrypter
					.encrypt(refreshInterval));
			properties.setProperty(propertyEncrypter
					.encrypt("CLIENT_LOOK_AND_FEEL"), valueEncrypter
					.encrypt(applicationLookAndFeel));
			properties.setProperty(propertyEncrypter
					.encrypt("ENABLE_AUDIO_ADVISOR"), valueEncrypter
					.encrypt(Boolean.toString(enableAudioAdvisor)));
			properties.setProperty(
					propertyEncrypter.encrypt("IS_LOCAL_FOLDER"),
					valueEncrypter.encrypt(Boolean.toString(isLocalFolder)));
			properties.setProperty(
					propertyEncrypter.encrypt("CIFS_SHARE_PATH"),
					valueEncrypter.encrypt(cifsSharePath));
			properties.setProperty(propertyEncrypter
					.encrypt("CIFS_SHARE_DOMAIN"), valueEncrypter
					.encrypt(cifsShareDomain));
			properties.setProperty(propertyEncrypter.encrypt("CIFS_SHARE_IP"),
					valueEncrypter.encrypt(cifsShareLocation));
			properties.setProperty(propertyEncrypter
					.encrypt("CIFS_SHARE_PASSWORD"), valueEncrypter
					.encrypt(cifsSharePassword));
			properties.setProperty(propertyEncrypter
					.encrypt("CIFS_SHARE_USERNAME"), valueEncrypter
					.encrypt(cifsShareUsername));
			...
			properties.setProperty(propertyEncrypter
					.encrypt("ENABLE_CUSTOM_DESTINATION_FOLDER"),
					valueEncrypter.encrypt(Boolean
							.toString(enableCustomDestinationFolder)));
			storeSettings();
		} catch (GeneralSecurityException e) {
			error.launch(e, getClass());
		} catch (IOException e) {
	...
	public void writeItasaSettings() {
		try {
			...
			properties.setProperty(propertyEncrypter
					.encrypt("MYITASA_FEED_URL"), valueEncrypter
					.encrypt(myitasaFeedURL));
			properties.setProperty(propertyEncrypter
					.encrypt("MYITASA_USERNAME"), valueEncrypter
					.encrypt(myitasaUsername));
			properties.setProperty(propertyEncrypter
					.encrypt("MYITASA_PASSWORD"), valueEncrypter
					.encrypt(myitasaPassword));
			properties.setProperty(propertyEncrypter
					.encrypt("IS_AUTO_DOWNLOAD"), valueEncrypter
					.encrypt(Boolean.toString(isAutoDownload)));
			storeSettings();
		} catch (GeneralSecurityException e) {
			error.launch(e, getClass());
		} catch (IOException e) {
	...
	public void writeSubsfactorySettings() {
		try {
			properties.setProperty(propertyEncrypter
					.encrypt("SUBSFACTORY_FEED_URL"), valueEncrypter
					.encrypt(subsfactoryFeedURL));
			storeSettings();
		} catch (GeneralSecurityException e) {
			error.launch(e, getClass());
		} catch (IOException e) {
	...
	public void writeTorrentSettings() {
		try {
			properties.setProperty(propertyEncrypter
					.encrypt("TORRENT_DESTINATION_FOLDER"), valueEncrypter
					.encrypt(torrentDestinationFolder));
			storeSettings();
		} catch (GeneralSecurityException e) {
			error.launch(e, getClass());
		} catch (IOException e) {
	...
	/*
	 * public void writeFirstRunFalse() { isFirstTimeRun = false; try {
	 * properties.setProperty(propertyEncrypter.encrypt("IS_FIRST_TIME_RUN"),
	 * valueEncrypter.encrypt(Boolean.toString(isFirstTimeRun)));
	 * storeSettings(); } catch (GeneralSecurityException e) { error.launch(e,
	 * getClass()); } catch (IOException e) { error.launch(e, getClass(), null);
	 * } }
	 */
