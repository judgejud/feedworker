...
my @files = getFilesCompleted($id); #mi faccio restituire l'array dei files completati
if (scalar(@files)>0){ # se array > 0 quindi con almeno 1file completato
	...
	foreach my $item (@files) { #per ogni elemento dell'array files assegno e scorro
		...
		#creo la query
		my $query = "//ROLE[NAME/text()='$compose' and SEASON/text()='$season' and QUALITY/text()='$version']/PATH/text()";
		...
		if ($count>0){ #nodo trovato
 			my $path = XML::XPath::XMLParser::as_string($nodeset->get_nodelist); #restituisco il path 
 			my $newPath= "$dirTo$path/"; #compongo il nuovo path
 			if (-d $newPath) { #se esiste la directory path
	 			my $oldFile = "$dirFrom$item"; #compongo il percorso assoluto del file da spostare
	 			my $len = length($compose) + 1;
	 			my $newName = substr $item, $len; #tronca la prima parte del nome 
	 			my $newFile = "$newPath$newName"; #crea il nuovo percorso assoluto
